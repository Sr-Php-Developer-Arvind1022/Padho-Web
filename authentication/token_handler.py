...
def generate_access_token(user_id: int, username: str, role: str, force_logout_others: bool = True):
    ...
    if force_logout_others:
        # Optionally clear only user-specific tokens (uncomment if needed)
        # tokens_to_remove = {token for token in TOKEN_BLACKLIST if is_token_for_user(token, user_id)}
        # TOKEN_BLACKLIST.difference_update(tokens_to_remove)
        TOKEN_BLACKLIST.clear()  # Clears all tokens (modify if user-specific clearing is needed)
