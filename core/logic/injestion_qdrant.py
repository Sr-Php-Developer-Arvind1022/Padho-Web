...
def ingest_file(file_url: str, client_name: str, client_id: str, file_id: str, chroma_path: str = "./chroma_langchain_db"):
    ...
    try:
        print("Checking if collection already exists...")
        collectinfo = client.get_collection(collection_name=client_name)

        # check if dimension mismatch
        existing_size = collectinfo.vectors_count  # careful: this is count not size
        existing_dim = collectinfo.config.params.vectors.size

        if existing_dim != 768:
            print(f"Dimension mismatch ({existing_dim} vs 768). Recreating collection.")
            client.recreate_collection(
                collection_name=client_name,
                vectors_config=VectorParams(size=768, distance=Distance.COSINE)
            )
            print(f"Collection {client_name} recreated with 768 dimensions.")
        ...
        print(f"Collection {client_name} created with 768 dimensions.")
...
# Optional: add in batches to speed up first-time ingestion
    vstore.add_documents(documents=texts)
